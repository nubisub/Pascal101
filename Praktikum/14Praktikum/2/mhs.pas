...
label 
    awal,akhir;
var
    x,i,j,maks,mins,jumlah,k:integer;
    n:string;
    mhs : array [0..100] of mahasiswa;
    m:mahasiswa;
...
procedure search(x:integer);
    ...
    for j:=0 to x+1 do
    begin
        if mhs[j].nama=n then
        begin
            write('Data Ditemukan! Nilai Alpro ',n,' = ');
            writeln(mhs[j].nilai);
            break;
        end;
    end;

    if j=x+1 then writeln('Data Tidak Ditemukan!');
    end;

    end;

procedure sorting(x:integer);
begin
    if x = 0 then
        writeln('Anda Belum Memasukkan Data !!!')
    else
    begin
        ...
        begin
            for j := k+1 to x-1 do
            begin
                if mhs[k].nilai < mhs[j].nilai then
                begin
                    m:=mhs[k];
                    mhs[k]:=mhs[j];
                    mhs[j]:=m;
    ...
    writeln('Data Telah Diurutkan Berdasarkan Nilai!');

    end;
end;

procedure sorting2(x:integer);
begin
     if x = 0 then
        writeln('Anda Belum Memasukkan Data !!!')
    else
...
begin
    ...
    writeln('       7. Urutkan Nama');
    writeln('       8. Urutkan Nilai');
    writeln('       9. Keluar');
    writeln('===========================================');
